...
export function ImageUploadForm({ bookingId }: { bookingId: string }) {
    ...
    const uploadFile = async (file: File | null, bookingId: string) => {
        if (!file) return null;
        const filePath = `inspector/${bookingId}/checkin-${Date.now()}-${file.name}`;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await addInspectorFiles(bookingId, uploadedUrls, 'Check In');
            await updateBookingStatus(bookingId, 'Archived');
            
            toast({
                title: "Check-In Complete!",
                description: "The vehicle has been successfully checked in and the booking is archived.",
            });
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
             ...
             <Button type="submit" className="w-full" disabled={isUploading || files.every(f => f === null)}>
                ...
                {isUploading ? 'Uploading...' : 'Complete Check-In & Upload Pictures'}
            </Button>
        </form>
    )
}
